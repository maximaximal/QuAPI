...
static const char*
parse_dimacs(strictness strict,
             file* file,
             uint64_t* lineno_ptr,
             int* max_var_ptr) {
  *lineno_ptr = 1;
  ...
  int variables = ch - '0';
  while(isdigit(ch = NEXT())) {
    if(MAX_VARS / 10 < variables)
      return "maximum variable too large";
    variables *= 10;
    const int digit = ch - '0';
    if(MAX_VARS - digit < variables)
      return "maximum variable too large";
    variables += digit;
  }
  if(ch == EOF)
  ...
  for(;;) {
    ...
    if(ch == '-') {
      ...
      if(block_exists || block_forall) {
        return "previous quantifier block not closed!";
      }
      block_exists = true;
      ch = NEXT();
      continue;
    ...
    if(strict != RELAXED_PARSING && idx > variables)
      return nonl(
        ch, "maximum variable index exceeded " TRY_RELAXED_PARSING, lineno_ptr);
    if(idx) {
...
const char*
kissat_parse_dimacs(strictness strict,
                    file* file,
                    uint64_t* lineno_ptr,
                    int* max_var_ptr) {
  const char* res;
  res = parse_dimacs(strict, file, lineno_ptr, max_var_ptr);
  return res;
}
